...
#     })
def view_friends(request, username=None, template_name='friendship/friend/user_list.html'):
    """ View the friends of a user """
    if username:
        user = get_object_or_404(user_model, username=username)
    else:
...
@login_required
def friend_remove(request, to_username, template_name='friendship/friend/remove_friend.html'):
...
def friendship_request_list(request, template_name='friendship/friend/requests_list.html'):
    ...
    friendship_requests_from_others = Friend.objects.unread_requests(user=request.user)
    friendship_requests_sent = Friend.objects.sent_requests(user=request.user)
...
@login_required
def friendship_request_list_rejected(request, template_name='friendship/friend/requests_rejected_list.html'):
    """ View rejected friendship requests """
    friendship_requests=Friend.objects.rejected_requests(user=request.user)
...
def followers(request, username=None, template_name='friendship/follow/followers_list.html'):
...
def following(request, username=None, template_name='friendship/follow/following_list.html'):
...
def follower_add(request, followee_username):
    ...
    return render(request,'friendship/follow/add.html', ctx)
